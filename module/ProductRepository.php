...
class ProductRepository
{
    function register( array $params )
    {
        ...
        ( name, slug, description, price, price_offer, photo_main, photo_1, photo_2, photo_3 ) 
        VALUES 
        ( `{$params['name']}`, `{$params['slug']}`, `{$params['description']}`, `{$params['price']}`, `{$params['price_offer']}`, `{$params['photo_main']}`, `{$params['photo_1']}`, `{$params['photo_2']}`, `{$params['photo_3']}`  )
        " );
    }    
